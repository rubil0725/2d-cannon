...
int main()
{
   ...
   // constexpr int STRIP_COUNT = 16;
    ...
    // Demo player for the integrated try24 gameplay (position/angle/power)
    struct DemoPlayer {
        float x=0, y=0;
        float angle=45.0f;
        float power=300.0f;
    ...
    // Direct-hit uses shooterP->explosionRadius * 0.9f elsewhere; the
    // earlier standalone DIRECT_HIT_* constants were unused and removed.
                ...
                else
                {
                   ...
                   // previous landedhitonGround logic removed; no-op
                    if (proj.y >= groundY)
                    {
                        ...
                        // side-hit: use cannon hitbox vs explosion circle for more accurate overlap
                        if (CheckCollisionCircleRec({ explosion.x, explosion.y }, explosion.maxRadius, player1.cannon.GetHitbox()))
                            gameplay.applyDamage(player1, SIDEHIT_DAMAGE);
                        if (CheckCollisionCircleRec({ explosion.x, explosion.y }, explosion.maxRadius, player2.cannon.GetHitbox()))
                            gameplay.applyDamage(player2, SIDEHIT_DAMAGE);
                    }
                }
